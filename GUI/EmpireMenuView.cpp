...
void CEmpireMenuView::DrawEmpireSystemMenue(Graphics* g)
{
	...
	for (int i = 0; i < pMajor->GetEmpire()->GetSystemList()->GetSize(); i++)
	{
		...
		if (j < 21)
		{
			...
			s.Format("%i/%i",(KO.y),KO.x);
			// (ok) MYTRACE("logdata")(MT::LEVEL_DEBUG, "Empire System View ------------------------------------");
			// (ok) MYTRACE("logdata")(MT::LEVEL_DEBUG, "Empire System View - Coordinates System: %s \n", s);
			...
			// (ok) MYTRACE("logdata")(MT::LEVEL_DEBUG, "Empire System View - System: %s \n", s);
			...
			if (m_iSystemSubMenue == EMPIREVIEW_SYSTEMS_NORMAL)
			{
				...
				*/
				...
				s += moral;
				// (ok) MYTRACE("logdata")(MT::LEVEL_DEBUG, "Empire System View - Morale: %s \n", s);
				...
				int foodstore = pDoc->GetSystem(KO.x, KO.y).GetFoodStore();
				s.Format("%d",foodstore);
				if (foodstore < 200)
					fontBrush.SetColor(Color(250,100,0));
					else 
						if (j == m_iClickedSystem)
					fontBrush.SetColor(markColor);
						else
					fontBrush.SetColor(normalColor);
				// (ok) MYTRACE("logdata")(MT::LEVEL_DEBUG, "Empire System View - Food Storage: %s \n", s);
				...
				// display workers
				int freeworkers = pDoc->GetSystem(KO.x, KO.y).GetWorker(WORKER::FREE_WORKER);
				int allworkers = pDoc->GetSystem(KO.x, KO.y).GetWorker(WORKER::ALL_WORKER);
				s.Format("(%d/%d)",freeworkers,allworkers);
				if (freeworkers < 0)
					fontBrush.SetColor(Color(250,100,0));
					else 
						if (j == m_iClickedSystem)
					...
					fontBrush.SetColor(normalColor);
				g->DrawString(CComBSTR(s), -1, &Gdiplus::Font(CComBSTR(fontName), fontSize), RectF(520,140+j*25,100,25), &fontFormat, &fontBrush);

				// Industrieproduktion anzeigen + free worker
				int ip = pDoc->GetSystem(KO.x, KO.y).GetProduction()->GetIndustryProd();
				//int freeworkers = pDoc->GetSystem(KO.x, KO.y).GetWorker(WORKER::FREE_WORKER);
				//int allworkers = pDoc->GetSystem(KO.x, KO.y).GetWorker(WORKER::ALL_WORKER);
				//s.Format("%d   (%d/%d)",ip,allworkers,freeworkers);
				s.Format("%d",ip);
				// (ok) MYTRACE("logdata")(MT::LEVEL_DEBUG, "Empire System View - IndustryProd: %s \n", s);
				//MYTRACE("logdata")(MT::LEVEL_DEBUG, "Empire System View - FreeWorker: %s \n", s);
				//MYTRACE("logdata")(MT::LEVEL_DEBUG, "Empire System View - AllWorker: %s \n", s);
				...
				s.Format("%d",cp);
				// (ok) MYTRACE("logdata")(MT::LEVEL_DEBUG, "Empire System View - CreditsProd: %s \n", s);
				...
				else if (id > 0 && id < 10000)
				{
					if (ip > 0 && !pDoc->GetBuildingInfo(id).GetNeverReady())
					{
						...
						if (RoundToBuild > 1)
						{
							...
							// (ok) MYTRACE("logdata")(MT::LEVEL_DEBUG, "building order: %s \n", s);
						}
						else
						{
			...
			else if (m_iSystemSubMenue == EMPIREVIEW_SYSTEMS_RESOURCE)
			{
				...
				for (int k = TITAN; k <= DERITIUM; k++)
				{
					...
					{
						Bitmap* graphic = NULL;
						// kleines Ressourcensymbol vor dem Wert zeichnen
						switch (k)
						{
					...
					s.Format("%d", pDoc->GetSystem(KO.x, KO.y).GetResourceStore(k));
												// (ok) MYTRACE("logdata")(MT::LEVEL_DEBUG, "resources: %s \n", s);
			...
			else if (m_iSystemSubMenue == EMPIREVIEW_SYSTEMS_DEFENCE)
			{
				...
				MYTRACE("logdata")(MT::LEVEL_DEBUG, "troops: %s \n", s);
				...
				MYTRACE("logdata")(MT::LEVEL_DEBUG, "ShieldPower: %s\n", s);
				...
				MYTRACE("logdata")(MT::LEVEL_DEBUG, "ShieldDefence: %s\n", s);
				...
				MYTRACE("logdata")(MT::LEVEL_DEBUG, "GroundDefend: %s\n", s);
			...
			else if (m_iSystemSubMenue == EMPIREVIEW_SYSTEMS_TRADE)
			{
				...
				if (!pDoc)
					return;
				//habitants = pDoc->GetSystem(p.x, p.y).GetHabitants() / TRADEROUTEHAB);
			...
			else if (m_iSystemSubMenue == EMPIREVIEW_SYSTEMS_ENERGY) 
			{
				...
				for (int l = 0; l < pDoc->GetSystem(KO.x, KO.y).GetAllBuildings()->GetSize(); l++)
				{
					...
					int nID = pDoc->GetSystem(KO.x, KO.y).GetAllBuildings()->GetAt(l).GetRunningNumber();
					CBuildingInfo* pBuildingInfo = &pDoc->GetBuildingInfo(nID);
					if (pBuildingInfo->GetNeededEnergy() > 0 )
					{
						nNeededEnergy++;
						if (pDoc->GetSystem(KO.x, KO.y).GetAllBuildings()->GetAt(l).GetIsBuildingOnline() || pBuildingInfo->GetNeededEnergy() == 0 || pBuildingInfo->GetAllwaysOnline())
							nNeededEnergy++;
					}
					if (pBuildingInfo->GetEnergyBoni() > 0)
					{
						nEnergyBoni++;
						//if (pDoc->GetSystem(KO.x, KO.y).GetAllBuildings()->GetAt(l).GetIsBuildingOnline() || pBuildingInfo->GetNeededEnergy() == 0 || pBuildingInfo->GetAllwaysOnline())
						//	nShipDefenceOnlineCount++;
					}
					if (pBuildingInfo->GetResistance() > 0)
					{
						nResistance++;
						if (pDoc->GetSystem(KO.x, KO.y).GetAllBuildings()->GetAt(l).GetIsBuildingOnline() || pBuildingInfo->GetResistance() == 0 || pBuildingInfo->GetAllwaysOnline())
							nResistance++;
					}
				}

				// display of nNeededEnergy 
				s.Format("%d", nNeededEnergy);
				MYTRACE("logdata")(MT::LEVEL_DEBUG, "nNeededEnergy: %s \n", s);
				g->DrawString(CComBSTR(s), -1, &Gdiplus::Font(CComBSTR(fontName), fontSize), RectF(270,140+j*25,100,25), &fontFormat, &fontBrush);
				// Schildstärke anzeigen
				//s.Format("%d (%d/%d)", pDoc->GetSystem(KO.x, KO.y).GetProduction()->GetShieldPower(), nShieldDefenceOnlineCount, nShieldDefenceCount);
				//MYTRACE("logdata")(MT::LEVEL_DEBUG, "ShieldPower: %s\n", s);
				...
				//s.Format("%d (%d/%d)", pDoc->GetSystem(KO.x, KO.y).GetProduction()->GetResistance()(), nShipDefenceOnlineCount, nShipDefenceCount);
				//MYTRACE("logdata")(MT::LEVEL_DEBUG, "ShieldDefence: %s\n", s);
				//g->DrawString(CComBSTR(s), -1, &Gdiplus::Font(CComBSTR(fontName), fontSize), RectF(520,140+j*25,150,25), &fontFormat, &fontBrush);
				// display GetIncomeOnTradeRoutes()
				s.Format("%d (%d/%d)", pDoc->GetSystem(KO.x, KO.y).GetProduction()->GetIncomeOnTradeRoutes(), nNeededEnergy, nEnergyBoni);
				MYTRACE("logdata")(MT::LEVEL_DEBUG, "GroundDefend: %s\n", s);
				g->DrawString(CComBSTR(s), -1, &Gdiplus::Font(CComBSTR(fontName), fontSize), RectF(670,140+j*25,150,25), &fontFormat, &fontBrush);
				// Scanstärke anzeigen
				//s.Format("%d", pDoc->GetSystem(KO.x, KO.y).GetProduction()->GetScanPower());
			...
			} // END of if (m_iSystemSubMenue == EMPIREVIEW_SYSTEMS_ENERGY) 
	...
	if (m_iSystemSubMenue == EMPIREVIEW_SYSTEMS_NORMAL)
	{
		...
		g->DrawString(CComBSTR("fW"), -1, &font, RectF(530,110,100,30), &fontFormat, &fontBrush);
	...
	{
		g->DrawString(CComBSTR(CLoc::GetString("BTN_TRADEROUTE")), -1, &font, RectF(270,110,100,30), &fontFormat, &fontBrush);
		g->DrawString(CComBSTR(CLoc::GetString("BTN_RESOURCEROUTE")), -1, &font, RectF(370,110,150,30), &fontFormat, &fontBrush);
		g->DrawString(CComBSTR(CLoc::GetString("CURRENT_HABITANTS")), -1, &font, RectF(520,110,150,30), &fontFormat, &fontBrush);
		g->DrawString(CComBSTR(CLoc::GetString("MAX_HABITANTS")), -1, &font, RectF(670,110,150,30), &fontFormat, &fontBrush);
		g->DrawString(CComBSTR(CLoc::GetString("BTN_BLOCKADE_SYSTEM")), -1, &font, RectF(820,110,150,30), &fontFormat, &fontBrush);
	}
		else if (m_iSystemSubMenue == EMPIREVIEW_SYSTEMS_TUTORIAL)
	{
		g->DrawString(CComBSTR(CLoc::GetString("BTN_NORMAL")), -1, &font, RectF(270,110,100,30), &fontFormat, &fontBrush);
		g->DrawString(CComBSTR(CLoc::GetString("SHIELDPOWER")), -1, &font, RectF(370,110,150,30), &fontFormat, &fontBrush);
		g->DrawString(CComBSTR(CLoc::GetString("SHIPDEFEND")), -1, &font, RectF(520,110,150,30), &fontFormat, &fontBrush);
		g->DrawString(CComBSTR(CLoc::GetString("GROUNDDEFEND")), -1, &font, RectF(670,110,150,30), &fontFormat, &fontBrush);
		g->DrawString(CComBSTR(CLoc::GetString("SCANPOWER")), -1, &font, RectF(820,110,150,30), &fontFormat, &fontBrush);
	...
	for (UINT i = 0; i < pMajor->GetShipHistory()->GetSizeOfShipHistory(); i++)
	{
		if (pMajor->GetShipHistory()->IsShipAlive(i))
		{
			...
			if (m_bShowAliveShips && j < 21) // Auf jede Seite gehen nur 21 Einträge, deshalb muss abgebrochen werden
			{
				...
				//MYTRACE("shipsdetails")(MT::LEVEL_DEBUG, "Empire Ships Overview - ShipName: %s \n", s);
...
//  display of troops onboard		g->DrawString(CComBSTR(s), -1, &Gdiplus::Font(CComBSTR(fontName), fontSize), RectF(225,140+j*25,150,25), &fontFormat, &fontBrush);
				...
				//MYTRACE("shipsdetails")(MT::LEVEL_DEBUG, "Empire Ships Overview - BuildRound: %s \n", s);
				...
				//MYTRACE("shipsdetails")(MT::LEVEL_DEBUG, "Empire Ships Overview - BuildInSector: %s \n", s);
				...
				//MYTRACE("shipsdetails")(MT::LEVEL_DEBUG, "Empire Ships Overview - CurrentSector: %s \n", s);
				...
				//MYTRACE("shipsdetails")(MT::LEVEL_DEBUG, "Empire Ships Overview - Experiance: %s \n", s);
				...
				//MYTRACE("shipsdetails")(MT::LEVEL_DEBUG, "Empire Ships Overview - CurrentTask: %s \n", s);
				...
				//MYTRACE("shipsdetails")(MT::LEVEL_DEBUG, "Empire Ships Overview - Target: %s \n", s);
				//MYTRACE("shipsdetails")(MT::LEVEL_DEBUG, "Empire Ships Overview ----------------------------");
	...
	MYTRACE("shipsdetails")(MT::LEVEL_INFO, "Empire Ships Overview - numberLive: %s \n", s);
	g->DrawString(CComBSTR(s), -1, &font, RectF(280,70,300,40), &fontFormat, &fontBrush);

	s.Format("%s: %d",CLoc::GetString("SHIPS_LOST"),numberDead);
	MYTRACE("shipsdetails")(MT::LEVEL_DEBUG, "Empire Ships Overview - numberDead: %s \n", s);
...
void CEmpireMenuView::DrawEmpireVictoryMenue(Gdiplus::Graphics *g)
{
	...
	if (pDoc->m_VictoryObserver.IsVictoryCondition(VICTORYTYPE_DIPLOMACY))
	{
		...
		MYTRACE("logdata")(MT::LEVEL_INFO, "VICTORYTYPE_DIPLOMACY - NEEDED: %s \n", s);
		...
		MYTRACE("logdata")(MT::LEVEL_INFO, "VICTORYTYPE_DIPLOMACY - WE: %s \n", s);
		...
		MYTRACE("logdata")(MT::LEVEL_INFO, "VICTORYTYPE_DIPLOMACY - BEST: %s \n", s);
	...
	if (pDoc->m_VictoryObserver.IsVictoryCondition(VICTORYTYPE_CONQUEST))
	{
		...
		MYTRACE("logdata")(MT::LEVEL_INFO, "VICTORYTYPE_CONQUEST - NEEDED: %s \n", s);
		...
		MYTRACE("logdata")(MT::LEVEL_INFO, "VICTORYTYPE_CONQUEST - WE: %s \n", s);
		...
		MYTRACE("logdata")(MT::LEVEL_INFO, "VICTORYTYPE_CONQUEST - BEST: %s \n", s);
	...
	if (pDoc->m_VictoryObserver.IsVictoryCondition(VICTORYTYPE_RESEARCH))
	{
		...
		MYTRACE("logdata")(MT::LEVEL_INFO, "VICTORYTYPE_RESEARCH - NEEDED: %s \n", s);
		...
		MYTRACE("logdata")(MT::LEVEL_INFO, "VICTORYTYPE_RESEARCH - WE: %s \n", s);
		...
		MYTRACE("logdata")(MT::LEVEL_INFO, "VICTORYTYPE_RESEARCH - BEST: %s \n", s);
	...
	if (pDoc->m_VictoryObserver.IsVictoryCondition(VICTORYTYPE_COMBATWINS))
	{
		...
		MYTRACE("logdata")(MT::LEVEL_INFO, "VICTORYTYPE_COMBATWINS - NEEDED: %s \n", s);
		...
		MYTRACE("logdata")(MT::LEVEL_INFO, "VICTORYTYPE_COMBATWINS - WE: %s \n", s);
		...
		MYTRACE("logdata")(MT::LEVEL_INFO, "VICTORYTYPE_COMBATWINS - BEST: %s \n", s);
	...
	if (pDoc->m_VictoryObserver.IsVictoryCondition(VICTORYTYPE_SABOTAGE))
	{
		...
		MYTRACE("logdata")(MT::LEVEL_INFO, "VICTORYTYPE_SABOTAGE - NEEDED: %s \n", s);
		...
		MYTRACE("logdata")(MT::LEVEL_INFO, "VICTORYTYPE_SABOTAGE - WE: %s \n", s);
		...
		MYTRACE("logdata")(MT::LEVEL_INFO, "VICTORYTYPE_SABOTAGE - BEST: %s \n", s);
...
void CEmpireMenuView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	// Wenn wir in der Nachrichtenansicht sind
	if (m_iSubMenu == EMPIREVIEW_NEWS)
	{
		...
		if (ButtonReactOnLeftClick(point, &m_EmpireNewsFilterButtons, temp))
		{
			...
			if (m_iWhichNewsButtonIsPressed == 2)
			{
				temp = -1;
				m_iWhichNewsButtonIsPressed = (EMPIRE_NEWS_TYPE::Typ)temp; // TUTORIAL
			}
			return;
		}
...
void CEmpireMenuView::CreateButtons()
{
	...
	m_EmpireNewsFilterButtons.Add(new CMyButton(CPoint(377,70), CSize(160,40), "TUTORIAL",  fileN, fileI, fileA));
	...
	m_EmpireSystemFilterButtons.Add(new CMyButton(CPoint(540,70), CSize(160,40), CLoc::GetString("BTN_TRADE"),  fileN, fileI, fileA));
	m_EmpireSystemFilterButtons.Add(new CMyButton(CPoint(700,70), CSize(160,40), "TUTORIAL",  fileN, fileI, fileA)); // later build into Stringtable
	m_EmpireSystemFilterButtons.Add(new CMyButton(CPoint(860,70), CSize(160,40), CLoc::GetString("BTN_ENERGYMENUE"),  fileN, fileI, fileA));
