...
/////////////////////////////////////////////////////////////////////////////////////////
	...
	if (m_bySubMenu == 0)
		...
		s = "Ingame";  // later get from Stringtable.txt
	else if (m_bySubMenu == 4)
		s = "Database"; // later get from Stringtable.txt
...
void CDiplomacyMenuView::DrawRaceDiplomacyMenue(Graphics* g)
{
	...
	// draw Button DisplayAllRaces
		Bitmap* btnDisplayAllRaces = pDoc->GetGraphicPool()->GetGDIGraphic("Other\\" + pPlayer->GetPrefix() + "button_small.bop");//All-Button zeichnen
	if(btnDisplayAllRaces)
		g->DrawImage(btnDisplayAllRaces,40,650,80,30);

	if (!m_bDisplayAllRaces)
		s=CLoc::GetString("BTN_ALL");
	else
		s=CLoc::GetString("BTN_CURRENTS");
	g->DrawString(CComBSTR(s), -1, &Gdiplus::Font(CComBSTR(fontName), fontSize), RectF(40,650,80,30), &fontFormat, &fontBrush);
	...
	if (m_bySubMenu != 2)
	{
		...
		for (vector<CRace*>::iterator it = m_vRaceList.begin(); it != m_vRaceList.end(); ++it)
		{
			...
			if (m_sClickedOnRace == pRace->GetRaceID())
			{
				...
				//MYTRACE("diplomacy")(MT::LEVEL_DEBUG, "Diplomacy with %s, relation (0 to 100) = %d\n", name, pRace->GetRelation(pPlayer->GetRaceID()));
				if (graphic == NULL)
					graphic = pDoc->GetGraphicPool()->GetGDIGraphic("Races\\ImageMissing.boj");
				if (graphic)
					g->DrawImage(graphic, 735, 100, 300, 300);

				// (ok) MYTRACE("diplomacy")(MT::LEVEL_DEBUG, "Diplomacy with %s, relation (0 to 100) = %d\n", name, pRace->GetRelation(pPlayer->GetRaceID()));
				...
				{
					// (see above) MYTRACE("diplomacy")(MT::LEVEL_DEBUG, "relation (0 to 100) = %d\n", pRace->GetRelation(pPlayer->GetRaceID()));
					for (int t = 0; t < 20; t++)
					{
					...
					//MYTRACE("diplomacy")(MT::LEVEL_DEBUG, "relation (0 to 100) = %d\n", relation);
		...
		for (vector<CDiplomacyInfo*>::const_iterator it = m_vIncomeList.begin(); it != m_vIncomeList.end(); ++it)
		{
			...
			if (m_pIncomingInfo == *it)
			{
				...
				if (pRace->IsMinor() || pRace->IsMajor() && ((CMajor*)pRace)->IsHumanPlayer() == false)
				{
					...
					CString str_relation2;
					str_relation2.Format(" (%d)",relation);
						//MYTRACE("logevent")(MT::LEVEL_INFO, "str_relation: %s \n", str_relation);
					s = s + str_relation2;
...
void CDiplomacyMenuView::CreateButtons()
{
	...
	m_DiplomacyMainButtons.Add(new CMyButton(CPoint(580,690) , CSize(160,40), "ingame", fileN, fileI, fileA));
	m_DiplomacyMainButtons.Add(new CMyButton(CPoint(770,690) , CSize(160,40), "Database", fileN, fileI, fileA));
