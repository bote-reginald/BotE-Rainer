...
void CSmallInfoView::OnDraw(CDC* pDC)
{
		...
		&& m_DisplayMode == DISPLAY_MODE_SHIP_BOTTEM_VIEW || m_DisplayMode == DISPLAY_MODE_FLEET_MENU_VIEW)
	{
		...
		else if (!pShip->second->HasFleet() || pShip->second->LeaderIsCurrent())
		{
			...
			s.Format("%s (%s %d)",pShip->second->GetShipName(), "exp.", pShip->second->GetCrewExperience());
		...
		// Wenn wir Informationen zur Flotte anzeigen
		else
		{
		...
		if (TargetKO.x != -1 && pShip->second->GetOwnerOfShip() == pMajor->GetRaceID())
		{
			...
			if (rounds > 1)
				s.Format("%s: %i/%i (%d %s)", CLoc::GetString("TARGET"), (TargetKO.y),TargetKO.x, rounds, CLoc::GetString("ROUNDS"));
			else
				s.Format("%s: %i/%i (%d %s)", CLoc::GetString("TARGET"), (char)(TargetKO.y),TargetKO.x, rounds, CLoc::GetString("ROUND"));
		}
		if (TargetKO.x == -1 && pShip->second->GetOwnerOfShip() == pMajor->GetRaceID())
