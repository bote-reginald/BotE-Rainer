...
void CRandomEventCtrl::GlobalEventResearch(CMajor *pRace)
{
	CString messagetext=CLoc::GetString("GLOBALEVENTRESEARCH");
		MYTRACE("logevent")(MT::LEVEL_DEBUG, "RandomEventCtrl.CPP: GLOBALEVENTRESEARCH: FP before:%i, Empire:%s\n",pRace->GetEmpire()->GetFP(), pRace->GetEmpire());
	pRace->GetEmpire()->AddFP((int)(pRace->GetEmpire()->GetFP()));
	MYTRACE("logevent")(MT::LEVEL_DEBUG, "RandomEventCtrl.CPP: GLOBALEVENTRESEARCH: AddFP:%i, Empire:%s\n",pRace->GetEmpire()->GetFP(), pRace->GetEmpire());
...
void CRandomEventCtrl::CalcExploreEvent(const CPoint &ko, CMajor *pRace, CShipMap* ships)
{
	...
	if (eventnumber == ALIENTEC)
	{
		...
		MYTRACE("logevent")(MT::LEVEL_DEBUG, "RandomEventCtrl.CPP: ALIENTEC: Added FP: nAdd:%i, nSystemCount:%i * (50 to 150)\n",nAdd, nSystemCount);
		pRace->GetEmpire()->AddFP(nAdd);

		CString sSectorName = pDoc->GetSector(ko.x, ko.y).GetName(false);
		if (sSectorName.IsEmpty())
			sSectorName.Format("%i/%i", (ko.y), ko.x);
	...
	else if(eventnumber == EVENTSHIPXP)
	{
		for (CShipMap::iterator i = ships->begin(); i != ships->end(); ++i)
		{
			if (i->second->GetOwnerOfShip() == pRace->GetRaceID() && i->second->GetKO()==ko)
			{
				...
				MYTRACE("logevent")(MT::LEVEL_DEBUG, "RandomEventCtrl.CPP: EVENTSHIPXP: Added additional_experience:%i (50 to 450)\n",additional_experience);
			}
		}
	...
	for (int x = 0; x < STARMAP_SECTORS_HCOUNT; x++)
	{
		for (int y = 0; y < STARMAP_SECTORS_VCOUNT; y++)
		{
			...
			for (map<CString, CMajor*>::const_iterator it = pmMajors->begin(); it != pmMajors->end(); ++it)
			{
				...
				if (sSectorName.IsEmpty())
					sSectorName.Format("%i/%i", (pSector->GetKO().y), pSector->GetKO().x);
