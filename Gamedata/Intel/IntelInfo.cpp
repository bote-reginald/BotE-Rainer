		...
		for (int x = 0; x < STARMAP_SECTORS_HCOUNT; x++)
			for (map<CString, CMajor*>::const_iterator it = pmMajors->begin(); it != pmMajors->end(); ++it)
			{
				...
				if (sectors[x][y][it->first] > NULL)
				{
					...
					//MYTRACE("logdata")(MT::LEVEL_INFO, "m_nControlledSectors[it->first]: %d \n", m_nControlledSectors[it->first]);
					if (sectors[x][y][it->first] == 3)
					{
						m_nInhabitedSystems[it->first]++;
						//MYTRACE("logdata")(MT::LEVEL_INFO, "m_nInhabitedSystems[it->first]: %d \n", m_nInhabitedSystems[it->first]);
						m_nOwnedSystems[it->first]++;
						//MYTRACE("logdata")(MT::LEVEL_INFO, "m_nOwnedSystems[it->first]: %d \n", m_nOwnedSystems[it->first]);
					}
				...
				// (wrong place?) MYTRACE("logdata")(MT::LEVEL_INFO, "Major??: \n");	
				// (wrong place?) MYTRACE("logdata")(MT::LEVEL_INFO, "m_nControlledSectors[it->first]: %d \n", m_nControlledSectors[it->first]);
				// (wrong place?) MYTRACE("logdata")(MT::LEVEL_INFO, "m_nOwnedSystems[it->first]: %d \n", m_nOwnedSystems[it->first]);
				// (wrong place?) MYTRACE("logdata")(MT::LEVEL_INFO, "m_nInhabitedSystems[it->first]: %d \n", m_nInhabitedSystems[it->first]);
				// (wrong place?) MYTRACE("logdata")(MT::LEVEL_INFO, "m_nKnownMinors[it->first]: %d \n", m_nKnownMinors[it->first]);
				// (wrong place?) MYTRACE("logdata")(MT::LEVEL_INFO, "m_nMinorMembers[it->first]: %d \n", m_nMinorMembers[it->first]);
			}
	m_bCalculated = TRUE;
}
