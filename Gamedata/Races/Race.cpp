...
void CRace::Serialize(CArchive &ar)
{
	...
	{
		ar << m_sID;			// Rassen-ID
		//MYTRACE("logsave")(MT::LEVEL_DEBUG, "RaceID = m_sID: %s\n", m_sID);
		ar << m_sHomeSystem;	// Name des Heimatsystems
		//MYTRACE("logsave")(MT::LEVEL_DEBUG, "m_sHomeSystem: %s\n", m_sHomeSystem);
		ar << m_sName;			// Rassenname
		//MYTRACE("logsave")(MT::LEVEL_DEBUG, "m_sName: %s\n", m_sName);
...
/// Funktion erstellt eine Tooltipinfo der Rasse (in RaceList)
///   (another tooltip in DiplomacyMenuView (Tooltip for race image)
///   (another tooltip in EventFirstContact (RaceDescription-Tooltip)
