...
CString CShip::GetCurrentTargetAsString() const
{
	...
	if (!HasTarget())
		...
		target.Format("%i/%i", (m_TargetKO.y),m_TargetKO.x);
	return target;
}
...
UINT CShip::GetCompleteOffensivePower(bool bBeams/* = true*/, bool bTorpedos/* = true*/) const
{
		...
		for (int i = 0; i < m_BeamWeapons.GetSize(); i++)
		{
			...
			for (int j = 0; j < 100; j++)
			{
				...
				if (counter > m_BeamWeapons.GetAt(i).GetRechargeTime())
				{
					...
					//MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Ship.cpp: tempBeamDmg=%i, bHalfDmg=%i, DAMAGE_TO_HULL=0.1(allways)\n", tempBeamDmg, bHalfDmg);
					...
					//MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Ship.cpp: tempBeamDmgAfterPiercing(*1,5)=%i\n", tempBeamDmg);
					if (m_BeamWeapons.GetAt(i).GetModulating())
						tempBeamDmg *= 3;
					else if (bHalfDmg)
						tempBeamDmg /= 2;
					//MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Ship.cpp: tempBeamDmgAfterModulating(*3 else if halfdmg /2)=%i\n", tempBeamDmg);

					beamDmg += tempBeamDmg;
					//MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Ship.cpp: BeamDmg=%i (tempBeamDmg=%i)\n", beamDmg, tempBeamDmg);
				}
				counter--;
			}

			beamDmg = (UINT)((double)beamDmg * dTypeBonus);
			//MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Ship.cpp: BeamDmgAfterdTypeBonus=%i (dTypeBonus=%d)\n---------\n", beamDmg, dTypeBonus);
		}
		beamDmg /= 2;
		//MYTRACE("logships")(MT::LEVEL_DEBUG, "Ship.cpp: BeamDmg=%i \n", beamDmg);
	}
	// (ok) MYTRACE("logships")(MT::LEVEL_DEBUG, "Ship.cpp: BeamDmg=%i \n", beamDmg);
	...
	if (bTorpedos)
	{
		...
		// (ok) MYTRACE("logships")(MT::LEVEL_DEBUG, "Ship.cpp: torpedoDmg=%i \n", torpedoDmg);
	}
	...
	if (IsStation())
	{
		...
		//MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Ship.cpp: isStation: beamDmg+torpedoDmg *1,5 !)\n");
	}
	...
	MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "-----------------------\n");
	MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Ship.cpp: Ship: %s, Class: %s, CrewExp:%d, Offensiv: Cloak=%g (1=no), dMan=%g, beamDmg=%i, torpedoDmg=%i\n", 
		m_strShipName, m_strShipClass, m_iCrewExperiance, dCloak, dMan, beamDmg, torpedoDmg);
	// (ok, see above) MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Ship.cpp: Offensiv: beamDmg=%i, torpedoDmg=%i\n---------\n", beamDmg, torpedoDmg);
...
UINT CShip::GetCompleteDefensivePower(bool bShields/* = true*/, bool bHull/* = true*/) const
{
	...
	if (bHull)
	{
		def += m_Hull.GetMaxHull();
			// 8ok, see below) MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "SHIP.CPP: defence=%i, (m_Hull.GetMaxHull()=%i)\n", def, m_Hull.GetMaxHull());
		if (m_Hull.GetAblative())
			def = (UINT)(def * 1.3);
		//MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "SHIP.CPP: defence=%i (x 1,3 if Hull=Ablative)\n", def);
		if (m_Hull.GetPolarisation())
			def = (UINT)(def * 1.3);
		//MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "SHIP.CPP: defence=%i (x 1,3 if Hull=Polarisation)\n", def);
		// Hülle gilt doppelt soviel wie Schilde
		def *= 2;
		MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "SHIP.CPP: MaxHull()=%i, Ablative(x1.3):%s, Pola(x1.3): %s, defence=%i (x 2 -always, because Hull is double to shields)\n",
			m_Hull.GetMaxHull(), m_Hull.GetAblative() ? "true" : "false", m_Hull.GetPolarisation() ? "true" : "false", def);
	}

	if (bShields)
	{
		def += m_Shield.GetMaxShield() + (m_Shield.GetMaxShield() / 300 + 2 * m_Shield.GetShieldType()) * 100;
		// (ok, but double) MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "SHIP.CPP: defence=%i, (MaxShields()=%i), Shieldtype=%d\n", def, m_Shield.GetMaxShield(), m_Shield.GetShieldType());
	}
	...
	MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Ship.cpp: Defensiv=%i: dCloak=%g (1.0=NoCloak,1.25=CloakOn), m_byManeuverability=%d(-4/10*1.75+1)=->dMan=%g, MaxShields=%i, Shieldtype=%d\n", 
		def, dCloak, m_byManeuverability, dMan, m_Shield.GetMaxShield(), m_Shield.GetShieldType());
	//MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Ship.cpp: Defensiv=%i, dMan=%g, dCloak=%g\n---------\n", def, dMan, dCloak);
...
CString CShip::GetTooltip(const FleetInfoForGetTooltip* const info)
{
	...
	if (sName.IsEmpty() == false)
	{
		if (info)
		{
			// Schiffsnamen holen und die ersten 4 Zeichen (z.B. USS_) und die letzten 2 Zeichen (z.B. _A) entfernen
	...
	CString sStorageRoom;
	if (GetStorageRoom() != 0) 	{
	sStorageRoom.Format("%s: %d\n", CLoc::GetString("SHIP_STORAGE"), GetStorageRoom());
	sStorageRoom = CHTMLStringBuilder::GetHTMLColor(sStorageRoom);
	sStorageRoom = CHTMLStringBuilder::GetHTMLHeader(sStorageRoom, _T("h5"));
	sStorageRoom = CHTMLStringBuilder::GetHTMLCenter(sStorageRoom);
	sStorageRoom += CHTMLStringBuilder::GetHTMLStringNewLine();
	sStorageRoom += CHTMLStringBuilder::GetHTMLStringHorzLine();
	sStorageRoom += CHTMLStringBuilder::GetHTMLStringNewLine();
	};
	...
	CString sTip = sName + sType + sCosts + sMovementHead + sMovement + sBeamWeaponHead + sBeams + sTupeWeaponHead + sTorps + sDefensiveHead + sShield + sHull + sScanHead + sScan + sScanpower + sSpecialsHead + sSpecials + sStorageRoom + sDesc;
	return sTip;
}
