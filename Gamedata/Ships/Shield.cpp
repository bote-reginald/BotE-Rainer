...
void CShield::RechargeShields(int nMulti)
{
	...
	// (ok, see below) MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Shield recharge = %i \n", Recharge);
	m_iCurrentShield += Recharge;
	if (Recharge > 0)
		MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Shield recharge = %i, CurrentShield = %i \n", Recharge, m_iCurrentShield);
