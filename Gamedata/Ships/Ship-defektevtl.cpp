...
void CShip::Serialize(CArchive &ar)
{
	...
	{
		ar << m_iID;
		//MYTRACE("logsave")(MT::LEVEL_INFO, "Ship.CPP: Ship-m_iID: %s", m_iID);
		ar << m_KO;
		//MYTRACE("logsave")(MT::LEVEL_DEBUG, "Ship-m_KO: %s", m_KO);
		ar << m_TargetKO;
		ar << m_sOwnerOfShip;
...
UINT CShip::GetCompleteOffensivePower(bool bBeams/* = true*/, bool bTorpedos/* = true*/) const
{
		...
		for (int i = 0; i < m_BeamWeapons.GetSize(); i++)
		{
			...
			for (int j = 0; j < 100; j++)
			{
				...
				if (counter > m_BeamWeapons.GetAt(i).GetRechargeTime())
				{
					...
					MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Ship.cpp: tempBeamDmg=%i, bHalfDmg=%i, DAMAGE_TO_HULL=%d\n", tempBeamDmg, bHalfDmg, DAMAGE_TO_HULL);
			...
			MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Ship.cpp: BeamDmgAfterdTypeBonus=%i (*dTypeBonus=%i)\n---------\n", beamDmg, dTypeBonus);
		}
		beamDmg /= 2;
	}
	...
	MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Ship.cpp: Offensiv: dCloak=%g (1.25=CloakOn, 1.0=NoCloak), dMan=%g\n", dCloak, dMan);
	MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Ship.cpp: Offensiv: beamDmg=%i, torpedoDmg=%i\n---------\n", beamDmg, torpedoDmg);
...
UINT CShip::GetCompleteDefensivePower(bool bShields/* = true*/, bool bHull/* = true*/) const
{
	...
	if (bHull)
	{
		def += m_Hull.GetMaxHull();
			MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "SHIP.CPP: defence=%i, (m_Hull.GetMaxHull()=%i)\n", def, m_Hull.GetMaxHull());
		...
		def *= 2;
		MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "SHIP.CPP: defence=%i (x 2 -always, because Hull is double to shields)\n", def);
	}

	if (bShields)
	{
		def += m_Shield.GetMaxShield() + (m_Shield.GetMaxShield() / 300 + 2 * m_Shield.GetShieldType()) * 100;
		MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "SHIP.CPP: defence=%i, (MaxShields()=%i), Shieldtype=%d\n", def, m_Shield.GetMaxShield(), m_Shield.GetShieldType());
	}
	...
	MYTRACE("shipdetails")(MT::LEVEL_DEBUG, "Ship.cpp: Defensiv: dCloak=%g (1.25=CloakOn, 1.0=NoCloak), m_byManeuverability=%d(-4/10*1.75+1)=->dMan=%g\n", dCloak, m_byManeuverability, dMan);
...
CString CShip::GetTooltip(const FleetInfoForGetTooltip* const info)
{
	...
	//if (GetStorageRoom() != 0)
	//{
	// (rainer: doesn't work)(if GetStorageRoom() > 0);
	// {
	...
	// (no!) sStorageRoom += CHTMLStringBuilder::GetHTMLStringNewLine();
	// (no!) sStorageRoom += CHTMLStringBuilder::GetHTMLStringNewLine();
	//
	//};
