...
void CSystem::BuildBuildingsForMinorRace(CSector* sector, BuildingInfoArray* buildingInfo, USHORT averageTechlevel, const CMinor* pMinor)
{
	...
	if (m_Buildings.GetSize() < 5)
	{
		...
		//* wenn zuwenige Gebäude vorhanden sind, so werden hier noch ein Industriegebäude hinzugefügt
...
BOOLEAN CSystem::CheckPlanet(CBuildingInfo* building, CSector* sector)
{
	...
	for (int i = 0; i < number && Ok == FALSE; i++)
	{
		...
		if (planet.GetColonized() || (building->GetBuildingName() == "Exo-Gas-Raffinerie")) // Gasraffinerie mit berücksichtigen, da Planet nicht kolonisiert!
		{
	...
	if (Ok == FALSE)// && (!(building->GetBuildingName() == "Exo-Gas-Raffinerie")))	// Haben also kein Planet im System gefunden, auf dem wir das Gebäude bauen könnten
		// und zugleich aber die Ausnahme für die Exo-Gas-Raffinerie geschaffen
		return 0;
	else
		return 1;
}
